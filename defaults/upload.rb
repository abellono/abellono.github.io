...
release = client.create_release("abellono/abellono.github.io", "#{name.downcase}-#{version}", {:target_commitish => "master", :name => "#{name} #{version}", :body => "Automatic release of #{name} version #{version}. Visit https://abellono.github.io/apps/#{name.downcase} to download this release."})
